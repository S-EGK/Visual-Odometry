...
class VisualOdometry():
    def __init__(self, data_dir):
        ...
        P1 = block * block * 8
        P2 = block * block * 32
        self.disparity = cv2.StereoSGBM_create(minDisparity=0, numDisparities=32, blockSize=block, P1=P1, P2=P2) # type: ignore
        self.disparities = [
            np.divide(self.disparity.compute(self.images_l[0], self.images_r[0]).astype(np.float32), 16)]
        self.fastFeatures = cv2.FastFeatureDetector_create() # type: ignore
    ...
    def track_keypoints(self, img1, img2, kp1, max_error=4):
        ...
        trackpoints1 = np.expand_dims(cv2.KeyPoint_convert(kp1), axis=1) # type: ignore

        # Use optical flow to find tracked counterparts
        trackpoints2, st, err = cv2.calcOpticalFlowPyrLK(img1, img2, trackpoints1, None, **self.lk_params) # type: ignore
    ...
    def estimate_pose(self, q1, q2, Q1, Q2, max_iter=100):
        ...
        for _ in range(max_iter):
            ...
            sample_idx = np.random.choice(range(q1.shape[0]), 6) # type: ignore
        ...
        # Get the rotation vector
        r = out_pose[:3]    # type: ignore
        # Make the rotation matrix
        R, _ = cv2.Rodrigues(r)
        # Get the translation vector
        t = out_pose[3:]    # type: ignore
...
def main():
    ...
    for i, gt_pose in enumerate(tqdm(vo.gt_poses, unit="poses")):
        if i < 1:
            ...
            cur_pose = np.matmul(cur_pose, transf)  # type: ignore
